...
const SessionView: React.FC<SessionViewProps> = ({ sessionId, config, onClose, onConnectionChange }) => {
  ...
  const [paneWidths, setPaneWidths] = useState<[number, number, number]>([40, 30, 30]);
  const containerRef = useRef<HTMLDivElement>(null);
  const draggingRef = useRef<number | null>(null); // which divider: 0 = between terminal & remote, 1 = between remote & local
  const startXRef = useRef(0);
  const startWidthsRef = useRef<[number, number, number]>([40, 30, 30]);
  ...
  useEffect(() => {
    connectToServer();
    return () => {
      if (backendSessionId && (window as any).go?.main?.App?.DisconnectSSH) {
        (window as any).go.main.App.DisconnectSSH(backendSessionId).catch(console.error);
      }
    };
  }, [sessionId, config]);
  ...
  const connectToServer = async () => {
    try {
      ...
      if ((window as any).go?.main?.App?.ConnectSSH) {
        const sid = await (window as any).go.main.App.ConnectSSH(config);
  ...
  const handleDisconnect = async () => {
    try {
      if (backendSessionId && (window as any).go?.main?.App?.DisconnectSSH) {
        await (window as any).go.main.App.DisconnectSSH(backendSessionId);
      }
      onConnectionChange?.(false);
      onClose();
    } catch (err) {
  ...
  const handleDownloadToLocal = async (remotePath: string, localDir: string) => {
    try {
      ...
      if ((window as any).go?.main?.App?.DownloadFile) {
        const result = await (window as any).go.main.App.DownloadFile(backendSessionId, remotePath, localDir);
  ...
  const handleUploadToRemote = async (localPath: string, remoteDir: string) => {
    try {
      ...
      if ((window as any).go?.main?.App?.UploadFile) {
        const result = await (window as any).go.main.App.UploadFile(backendSessionId, localPath, remoteDir);
