...
8.1. **Command Snippets Auto-Reload**: The SSH-only Commands pane reads snippets from `~/Library/Application Support/xterm-file-manager/command-snippets.json` (via `os.UserConfigDir()` on each platform). When that file is saved through `WriteLocalFile()`, backend emits `command-snippets:changed` so `CommandPanel.tsx` reloads immediately without restart.
